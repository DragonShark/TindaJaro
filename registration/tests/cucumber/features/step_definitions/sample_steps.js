...
this.Given(/that I am on the homepage/, function() {
    var mirrorUrl = url.resolve(process.env.ROOT_URL, "/");
    client.url(mirrorUrl);
  })

this.When(/I enter (\w+)/, function(classification){
  client.waitForExist('#discount');
  client.setValue('#classification', classification);
})
